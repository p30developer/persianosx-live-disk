...
edit = sed -e "s|@datadir[@]|$(DESTDIR)$(PREFIX)/share/persianosx-tools|g" \
	-e "s|@sysconfdir[@]|$(DESTDIR)$(SYSCONFDIR)/persianosx-tools|g" \
	-e "s|@libdir[@]|$(DESTDIR)$(PREFIX)/lib/persianosx-tools|g"
...
install_base:
	...
	install -dm0755 $(DESTDIR)$(PREFIX)/lib/persianosx-tools
	install -m0644 ${LIBS} $(DESTDIR)$(PREFIX)/lib/persianosx-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/persianosx-tools
	install -m0644 ${SHARED} $(DESTDIR)$(PREFIX)/share/persianosx-tools
...
uninstall_base:
	...
	for f in ${SHARED}; do rm -f $(DESTDIR)$(PREFIX)/share/persianosx-tools/$$f; done
	for f in ${LIBS}; do rm -f $(DESTDIR)$(PREFIX)/lib/persianosx-tools/$$f; done
...
dist:
	git archive --format=tar --prefix=persianosx-tools-livecd-$(Version)/ $(Version) | gzip -9 > persianosx-tools-livecd-$(Version).tar.gz
	gpg --detach-sign --use-agent persianosx-tools-livecd-$(Version).tar.gz
