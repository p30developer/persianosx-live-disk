...
load_live_config(){
	...
	echo "Loaded ${live_conf}: $(elapsed_time_ms ${livetimer})ms" >> /var/log/-live.log
...
configure_language(){
    ...
    locale-gen ${lang}
    echo "Configured language: ${lang}" >> /var/log/-live.log
    echo "Configured keymap: ${keytable}" >> /var/log/-live.log
    echo "Configured timezone: ${timezone}" >> /var/log/-live.log
}

configure_machine_id(){
	if [ -e "/etc/machine-id" ] ; then
		# delete existing machine-id
		echo "Deleting existing machine-id ..." >> /var/log/-live.log
		rm /etc/machine-id
	fi
	# set unique machine-id
	echo "Setting machine-id ..." >> /var/log/-live.log
